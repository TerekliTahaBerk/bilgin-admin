...
import { createUnitRequestSchema } from "@/contracts/admin/workflows";
...
export async function POST(request: NextRequest) {
  ...
  const input = createUnitRequestSchema.safeParse(body);
  if (!input.success) return createInvalidRequestResponse();
  const result = await adminWorkflows.createUnit(
    input.data,
